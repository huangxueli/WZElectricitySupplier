...
public class LKWindow extends BaseWindow{
	
	public final static String TAG = "LKWindow";
	
	private LKTable mLKTable;
	private Integer mPrimaryID;
	private Integer mGraphicID;
	private LKRecord mLKRecord;
	...
	// 新建
	public LKWindow(MainActivity mMainActivity, Point mPoint, int mGraphicID) {
		super(mMainActivity);
		this.mPoint = mPoint;
		this.mGraphicID = mGraphicID;
		mLKTable = mMainFragment.getLKTable();
		mCircuitTable = mMainFragment.getCircuitTable();
		isNewAdd = true;
	}
	// 已存在
	public LKWindow(MainActivity mMainActivity, Integer mPrimaryID, Point mPoint, int mGraphicID) {
		super(mMainActivity);
		this.mPoint = mPoint;
	...
	public void showWindow(final float screenx, final float screeny, final GraphicsLayer graphicsLayer){
		mView = mMainActivity.getLayoutInflater().inflate(R.layout.window_lk, null);
		...
		if(mPrimaryID !=null){
			mLKRecord = mLKTable.selectLineConnector(mPrimaryID);
		}
		if(mLKRecord!=null){
			String[] mNameParts = separateName(mLKRecord.mName);
			...
			mRemark.setText(mLKRecord.mRemark);
			String pictureStr = mLKRecord.mPicture;
...
//				refesh(mContainer, false);
			...
			String defectIDStr = mLKRecord.mDefectID;
		...
		mOKBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(mPrimaryID!=null){
					mDefectIDList = AppUtil.turnStringToList(mLKTable.selectLineConnector(mPrimaryID).mDefectID);
				...
				if(circuitid!=null){
					if(name==null){
						Util.ShowDialog(mMainActivity, "请填写完整的名称：线路名 + 令克名称！");
						HideKeyboard();
					}else{
						LKRecord record = new LKRecord(name, circuitid, picture, defectIds, remark, "否");
						record = new LKRecord(name, circuitid, picture, defectIds, remark, getEditState(mLKRecord, record));
						if(mPrimaryID == null){ 
							// 新增
							mPrimaryID = mLKTable.add(record, mPoint);
							ArcgisTool.updateGraphic(mGraphicID, graphicsLayer, mPrimaryID, AddMode.DeviceLK);
							ArcgisTool.addTextGraphic(mPoint, record.mName, mMap, mMainFragment.getDeviceTextLayer());
						}else{
							// 更新
							boolean b = mLKTable.update(mPoint, record, mPrimaryID);
							if(b && !mLKRecord.mName.equals(name)){ // 若名称被更改 同步起点或终点字段
								AppUtil.updateDXField(mMainFragment, mCircuitTable.getCircuitID(), name, mLKRecord.mName);
								AppUtil.updateDefectField(mMainFragment, name, mLKRecord.mName, mLKRecord.mDefectID);
							}
		...
		mDefectManageBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(mPrimaryID!=null){
					mDefectWindow = new DefectWindow(mMainActivity, mPrimaryID, DeviceType.DeviceLK);
				...
				DialogInterface.OnClickListener mPositiveListener = new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if(mPrimaryID!=null){
							...
							result = result && mLKTable.delete(mPrimaryID);
							if(result) {
								deleteRelatePicture();
...
//		mEditableSpinnerEdit.setEnabled(true);
	...
	private String getEditState(LKRecord oldone, LKRecord newone){
