...
//				refesh(mContainer, false);
		...
		mOKBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(circuitid!=null){
					if(name==null){
						Util.ShowDialog(mMainActivity, "请填写完整的名称：线路名 + 开闭所名称！");
						HideKeyboard();
					}else{
						KBRecord record = new KBRecord(name, circuitid, backupIntervalNum, outIntervalNum, inIntervalNum, picture, defectIds, remark, "否");
						...
						if(mPrimaryID == null){ 
							...
							if(b && !mKBRecord.mName.equals(name)){ // 若名称被更改 同步起点或终点字段
