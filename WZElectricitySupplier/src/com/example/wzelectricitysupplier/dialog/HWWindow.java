...
public class HWWindow extends BaseWindow {
	
	public final static String TAG = "HWWindow";
	
	private HWTable mHWTable;
	private Integer mPrimaryID;
	private Integer mGraphicID;
	private HWRecord mHWRecord;
	...
	// 新建
	public HWWindow(MainActivity mMainActivity, Point mPoint, int mGraphicID) {
		super(mMainActivity);
		this.mPoint = mPoint;
		this.mGraphicID = mGraphicID;
		mHWTable = mMainFragment.getHWTable();
		mCircuitTable = mMainFragment.getCircuitTable();
		isNewAdd = true;
	}
	// 已存在
	public HWWindow(MainActivity mMainActivity, Integer mPrimaryID, Point mPoint, int mGraphicID) {
		super(mMainActivity);
		this.mPoint = mPoint;
	...
	public void showWindow(final float screenx, final float screeny, final GraphicsLayer graphicsLayer){
		mView = mMainActivity.getLayoutInflater().inflate(R.layout.window_hw, null);
		...
		if(mPrimaryID !=null){
			mHWRecord = mHWTable.selectRingMainUnit(mPrimaryID);
		}
		if(mHWRecord!=null){
			String[] mNameParts = separateName(mHWRecord.mName);
			...
			mRemark.setText(mHWRecord.mRemark);
			mOutIntervalNum.setText(mHWRecord.mOutIntervalNum);
			mInIntervalNum.setText(mHWRecord.mInIntervalNum);
			mBackupIntervalNum.setText(mHWRecord.mBackupIntervalNum);
			
			String pictureStr = mHWRecord.mPicture;
...
//				refesh(mContainer, false);
			...
			String defectIDStr = mHWRecord.mDefectID;
		...
		mOKBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(mPrimaryID!=null){
					mDefectIDList = AppUtil.turnStringToList(mHWTable.selectRingMainUnit(mPrimaryID).mDefectID);
				...
				if(circuitid!=null){
					if(name==null){
						Util.ShowDialog(mMainActivity, "请填写完整的名称：线路名 + 环网单元名称！");
						HideKeyboard();
					}else{
						HWRecord record = new HWRecord(name, circuitid, backupIntervalNum, outIntervalNum, inIntervalNum, picture, defectIds, remark, "否");
						record = new HWRecord(name, circuitid, backupIntervalNum, outIntervalNum, inIntervalNum, picture, defectIds, remark, getEditState(mHWRecord, record));
						if(mPrimaryID == null){ 
							// 新增
							mPrimaryID = mHWTable.add(record, mPoint);
							ArcgisTool.updateGraphic(mGraphicID, graphicsLayer, mPrimaryID, AddMode.DeviceHW);
							ArcgisTool.addTextGraphic(mPoint, record.mName, mMap, mMainFragment.getDeviceTextLayer());
						}else{
							// 更新
							boolean b = mHWTable.update(mPoint, record, mPrimaryID);
							if(b && !mHWRecord.mName.equals(name)){ // 若名称被更改 同步起点或终点字段
								AppUtil.updateDXField(mMainFragment, mCircuitTable.getCircuitID(), name, mHWRecord.mName);
								AppUtil.updateDefectField(mMainFragment, name, mHWRecord.mName, mHWRecord.mDefectID);
							}
		...
		mDefectManageBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(mPrimaryID!=null){
					mDefectWindow = new DefectWindow(mMainActivity, mPrimaryID, DeviceType.DeviceHW);
				...
				DialogInterface.OnClickListener mPositiveListener = new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if(mPrimaryID!=null){
							...
							result = result && mHWTable.delete(mPrimaryID);
							if(result) {
								deleteRelatePicture();
...
//		mEditableSpinnerEdit.setEnabled(true);
	...
	private String getEditState(HWRecord oldone, HWRecord newone){
