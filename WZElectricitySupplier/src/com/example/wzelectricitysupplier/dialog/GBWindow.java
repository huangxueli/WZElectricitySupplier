...
//				refesh(mContainer, false);
		...
		mOKBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(circuitid!=null){
					if(name==null){
						Util.ShowDialog(mMainActivity, "请填写完整的名称：线路名 + 杆变名称！");
						HideKeyboard();
					}else{
						GBRecord record = new GBRecord(name, circuitid, capacity, picture, defectIds, remark, "否");
						...
						if(mPrimaryID == null){ 
							...
							if(b && !mGBRecord.mName.equals(name)){ // 若名称被更改 同步起点或终点字段
