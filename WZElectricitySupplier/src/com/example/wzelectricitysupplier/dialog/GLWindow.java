...
public class GLWindow extends BaseWindow implements OnCheckedChangeListener{
	...
	// 新建
	public GLWindow(MainActivity mMainActivity, Point mPoint, int mGraphicID) {
		super(mMainActivity);
		this.mPoint = mPoint;
		this.mGraphicID = mGraphicID;
		mGLTable = mMainFragment.getGLTable();
		mCircuitTable = mMainFragment.getCircuitTable();
		isNewAdd = true;
	}
	// 已存在
	public GLWindow(MainActivity mMainActivity, Integer mPrimaryID, Point mPoint, int mGraphicID) {
		super(mMainActivity);
		this.mPoint = mPoint;
	...
	public void showWindow(final float screenx, final float screeny, final GraphicsLayer graphicsLayer){
		mView = mMainActivity.getLayoutInflater().inflate(R.layout.window_gl, null);
		...
		if(mPrimaryID !=null){
			mGLRecord = mGLTable.selectDisconnectore(mPrimaryID);
		}
		if(mGLRecord!=null){
			String[] mNameParts = separateName(mGLRecord.mName);
			...
			mRemark.setText(mGLRecord.mRemark);
			mState.setText(mGLRecord.mState);
			decideWhichCheckedWithState(mGLRecord.mState);
			
			String pictureStr = mGLRecord.mPicture;
...
//				refesh(mContainer, false);
			...
			String defectIDStr = mGLRecord.mDefectID;
		...
		mOKBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(mPrimaryID!=null){
					mDefectIDList = AppUtil.turnStringToList(mGLTable.selectDisconnectore(mPrimaryID).mDefectID);
				...
				if(circuitid!=null){
					if(name==null){
						Util.ShowDialog(mMainActivity, "请填写完整的名称：线路名 + 隔离开关名称！");
						HideKeyboard();
					}else{
						GLRecord record = new GLRecord(name, circuitid, state, picture, defectIds, remark, "否");
						record = new GLRecord(name, circuitid, state, picture, defectIds, remark, getEditState(mGLRecord, record));
						if(mPrimaryID == null){ 
							// 新增
							mPrimaryID = mGLTable.add(record, mPoint);
							...
							// 更新
							boolean b = mGLTable.update(mPoint, record, mPrimaryID);
							if(b && !mGLRecord.mName.equals(name)){ // 若名称被更改 同步起点或终点字段
								AppUtil.updateDXField(mMainFragment, mCircuitTable.getCircuitID(), name, mGLRecord.mName);
								AppUtil.updateDefectField(mMainFragment, name, mGLRecord.mName, mGLRecord.mDefectID);
							}
		...
		mDefectManageBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(mPrimaryID!=null){
					mDefectWindow = new DefectWindow(mMainActivity, mPrimaryID, DeviceType.DeviceGL);
				...
				DialogInterface.OnClickListener mPositiveListener = new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if(mPrimaryID!=null){
							...
							result = result && mGLTable.delete(mPrimaryID);
							if(result) {
								deleteRelatePicture();
...
//		mEditableSpinnerEdit.setEnabled(true);
	...
	private String getEditState(GLRecord oldone, GLRecord newone){
