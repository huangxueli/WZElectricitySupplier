...
public class BasePhotoPreviewActivity extends Activity implements OnPageChangeListener, OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		overridePendingTransition(R.anim.photoselector_activity_alpha_action_in, 0); // 渐入效果
	...
	private OnClickListener photoItemClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if (!isUp) {
				new AnimationUtil(getApplicationContext(), R.anim.photoselector_translate_up)
						.setInterpolator(new LinearInterpolator()).setFillAfter(true).startAnimation(layoutTop);
				isUp = true;
			} else {
				new AnimationUtil(getApplicationContext(), R.anim.photoselector_translate_down_current)
						.setInterpolator(new LinearInterpolator()).setFillAfter(true).startAnimation(layoutTop);
				isUp = false;
			}
		}
